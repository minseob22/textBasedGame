...
Character party[MAX_PARTY_SIZE] = {
    {.job = "Warrior", .attacker = { .name = "Default Hero 1", .health = 1200, .max_health = 1200, .attack = 30, .speed = 60, .gauge = 0, .critical_rate = 15, .dodge_rate = 10 }}
};

int current_party_size = 1;

Character protagonist[] = {
    {.job = "Warrior", .attacker = { .name = "Default Hero 1", .health = 1200, .max_health = 1200, .attack = 30, .speed = 60, .gauge = 0, .critical_rate = 15, .dodge_rate = 10 }},
    {.job = "Mage", .attacker = { .name = "Stella", .health = 1000, .max_health = 1000, .attack = 40, .speed = 50, .gauge = 0, .critical_rate = 10, .dodge_rate = 7 }},
    {.job = "Archer", .attacker = { .name = "Ingi", .health = 900, .max_health = 900, .attack = 20, .speed = 70, .gauge = 0, .critical_rate = 30, .dodge_rate = 10 }},
    {.job = "Rogue", .attacker = { .name = "Yulih", .health = 1000, .max_health = 1000, .attack = 25, .speed = 70, .gauge = 0, .critical_rate = 20, .dodge_rate = 15 }}
};
...
void set_protagonist_name(const char *name) {
    snprintf(protagonist[0].attacker.name, sizeof(protagonist[0].attacker.name), "%s", name); // 이름 변경
    party[0] = protagonist[0];  // 이름 변경 후, party[0]에 반영
}
